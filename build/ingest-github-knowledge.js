...
export async function ingestGitHubKnowledge(options = {}) {
    const { sources = MDK_KNOWLEDGE_SOURCES, githubToken = process.env.GITHUB_TOKEN, useCache = true, vectorDbPath = "./knowledge.db", chunkSize = 1000, chunkOverlap = 200, } = options;
    ...
    try {
        ...
        for (const file of files) {
            try {
                ...
                const source = sources.find(s => file.url.includes(s.url.split("/").slice(3).join("/")));
        ...
        logger.info(`Vector database: build/embeddings/${path.basename(vectorDbPath)}.json and .bin`);
    }
    catch (error) {
