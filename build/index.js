...
import { runCommand, getModulePath, generateTemplateBasedMetadata, getServiceDataWithFallback, getMobileServiceAppNameWithFallback, getSchemaVersion, getServerConfig, safeJsonParse, } from "./utils.js";
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "mdk-create",
                description: "Creates MDK projects or entity metadata using templates (CRUD, List Detail, Base). Use this for initializing new projects or adding entity metadata to existing projects.",
                inputSchema: {
                    type: "object",
                    properties: {
                        folderRootPath: {
    ...
    switch (request.params.name) {
        case "mdk-create": {
            try {
                ...
                const script = await generateTemplateBasedMetadata(oDataEntitySetsString, templateType, projectPath, useOffline, isEntity);
                if (!script) {
