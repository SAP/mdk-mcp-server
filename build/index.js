...
import { runCommand, getModulePath, generateTemplateBasedMetadata, getServiceDataWithFallback, getMobileServiceAppNameWithFallback, getSchemaVersion, getServerConfig, safeJsonParse, geServiceMetadataJson, } from "./utils.js";
import { isCapProject, getCapMdkConfig, resolveMdkProjectPath, } from "./cap-utils.js";
import { validateToolArguments } from "./validation.js";
import { getCFToken, getMobileServicesAdminAPI, getCFAuthErrorMessage, isCFLoggedIn, refreshCFToken, } from "./cf-auth.js";
import { createMobileServicesClient } from "./mobile-services-client.js";
import path from "path";
import fs from "fs";
import { TelemetryHelper, unknownTool, } from "./telemetry/index.js";
import { retrieveAndStore, retrieveAndStoreRule, search, searchNames, searchRuleNames, getDocuments, printResults, } from "./vector.js";
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "mdk-create",
                description: "Creates MDK projects or entity metadata using templates (CRUD, List Detail, Base). Use this for initializing new projects or adding entity metadata to existing projects. Supports CAP projects - automatically creates MDK apps in the app/ folder with proper naming and configuration.",
                annotations: {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        scope: {
                            type: "string",
                            enum: ["project", "entity"],
                            description: "The scope of creation:\n" +
                        ...
                        templateType: {
                            ...
                            description: "The type of template to use. Note: 'base' template is only valid for project scope.",
                        },
                        oDataEntitySets: {
                            type: "string",
                            description: "The OData entity sets relevant to the user prompt, separated by commas.",
                        },
                        offline: {
                            type: "boolean",
                            description: "Whether to generate the project in offline mode (only applicable for project scope). Set to false unless offline is explicitly specified.",
                            default: false,
                        },
                        cfOrg: {
                            type: "string",
                            description: "Optional: Cloud Foundry organization name. If provided along with cfSpace, will be used to configure the deployment target.",
                        },
                        cfSpace: {
                            type: "string",
                            description: "Optional: Cloud Foundry space name. If provided along with cfOrg, will be used to configure the deployment target.",
                        },
                    },
                    required: [
                        "folderRootPath",
            ...
            {
                name: "mdk-gen",
                description: "Generates MDK artifacts including pages, actions, i18n files, and rule references. Returns prompts for LLM processing (pages, actions, i18n) or searches for rule examples.",
                annotations: {
                ...
                inputSchema: {
                    ...
                    properties: {
                        folderRootPath: {
                            type: "string",
                            description: "The path of the current project root folder (not required for rule artifact type).",
                        },
                        artifactType: {
                            type: "string",
                        ...
                        pageType: {
                            ...
                            description: "The type of page (required when artifactType is 'page'):\n" +
                        ...
                        controlType: {
                            ...
                            description: "The control type for databinding pages (required when pageType is 'databinding').",
                        },
                        oDataEntitySets: {
                            type: "string",
                            description: "Optional: The OData entity sets to use for page/action generation, separated by commas (required only when artifactType is 'action' or artifactType is 'page' and pageType is 'databinding').",
                        },
                        layoutType: {
                            type: "string",
                            enum: [
                            ...
                            description: "The layout type for layout pages (required when pageType is 'layout').",
                        },
                        actionType: {
                            type: "string",
                            enum: [
                            ...
                            description: "The type of action (required when artifactType is 'action').",
                        },
                        query: {
                            type: "string",
                            description: "Search query for rule reference (required only when artifactType is 'rule'). Examples: 'get app name', 'handle form validation', 'navigate to page', etc.",
                        },
                    },
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        externals: {
                            ...
                            description: "Optional: Array of external package names to include in the deployment (e.g., ['@nativescript/geolocation']). Defaults to empty array if not specified.",
                            default: [],
                        },
                    },
            ...
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        query: {
                            type: "string",
                            description: "Search query string (required for 'search' and 'search-samples' operations).",
                        },
                        component_name: {
                            type: "string",
            ...
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        folderRootPath: {
                            type: "string",
                            description: "The path of the MDK project root folder where .service.metadata will be saved.",
                        },
                        appId: {
                            type: "string",
                            description: "The Mobile Services application ID (e.g., 'myapp.mdk.demo').",
                        },
                        destination: {
                            type: "string",
                            description: "The destination name configured in Mobile Services (e.g., 'IncidentManagement').",
                        },
                        pathSuffix: {
                            type: "string",
                            description: "Optional path suffix to append before /$metadata (e.g., '/api/v1'). Leave empty if not needed.",
                            default: "",
                        },
                        landscapeType: {
                            type: "string",
                            enum: ["Standard", "Preview"],
                            description: "The Mobile Services landscape type. Use 'Standard' for production environments (default) or 'Preview' for preview/test environments.",
                            default: "Standard",
                        },
                    },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const projectPath = requestArgs.folderRootPath;
    const telemetryProperties = {
        tool: request.params.name,
    };
    await TelemetryHelper.sendTelemetry(isValidTool ? request.params.name : unknownTool, telemetryProperties, projectPath);
    switch (request.params.name) {
        case "mdk-create": {
            try {
                // Validate all arguments using comprehensive validation
                const validatedArgs = validateToolArguments("mdk-create", request.params.arguments || {});
                let projectPath = validatedArgs.folderRootPath;
                const scope = validatedArgs.scope;
                ...
                const cfOrg = validatedArgs.cfOrg;
                const cfSpace = validatedArgs.cfSpace;
                ...
                const script = await generateTemplateBasedMetadata(oDataEntitySetsString, templateType, projectPath, useOffline, isEntity, cfOrg, cfSpace);
                if (!script) {
        ...
        case "mdk-gen": {
            try {
                // Validate all arguments using comprehensive validation
                const validatedArgs = validateToolArguments("mdk-gen", request.params.arguments || {});
                const artifactType = validatedArgs.artifactType;
        ...
        `.trim();
                    ...
                    case "page": {
                        const projectPath = validatedArgs.folderRootPath;
                        const pageType = validatedArgs.pageType;
                        if (pageType === "databinding") {
                            const controlType = validatedArgs.controlType;
                            const oDataEntitySets = validatedArgs.oDataEntitySets;
        ...
        `;
                            ...
                            const serviceResult = await getServiceDataWithFallback(projectPath, oDataEntitySets);
                            if (!serviceResult) {
                                return {
                                    content: [
                                        {
        ...
        `;
          ...
          \`\`\`${MDK_EXAMPLE}\`\`\``;
                    ...
                    case "rule": {
                        ...
                        resultContent += `**Relevance Score:** ${(similarity * 100).toFixed(1)}%\n\n`;
                        try {
                            ...
                            resultContent += `*Error reading file: ${fileReadError instanceof Error
            ...
            try {
                ...
                switch (operation) {
                    ...
                    case "deploy": {
                        ...
                        let externals = validatedArgs.externals || [];
                        ...
                        if (externals.length === 0) {
                            ...
                            if (fs.existsSync(vscodeSettingsPath)) {
                                try {
                                    ...
                                    const settings = (await safeJsonParse(settingsContent)); // eslint-disable-line @typescript-eslint/no-explicit-any
                                    if (settings["mdk.bundlerExternals"] &&
                                        Array.isArray(settings["mdk.bundlerExternals"])) {
                                        externals = settings["mdk.bundlerExternals"];
                                        console.error(`[MDK MCP Server] Using externals from .vscode/settings.json: ${externals.join(", ")}`);
                                    }
                                }
                                catch (error) {
                                    console.error(`[MDK MCP Server] Failed to read externals from .vscode/settings.json: ${error instanceof Error ? error.message : String(error)}`);
                                }
                            }
                        }
                        // Use getMobileServiceAppNameWithFallback to get mobile service app name with fallback logic
                        const mobileServiceAppName = await getMobileServiceAppNameWithFallback(projectPath);
                        ...
                        const isCapProjectScenario = isCapProject(path.dirname(projectPath));
                        ...
                        if (isCapProjectScenario) {
                            try {
                                ...
                                if (fs.existsSync(serviceMetadataPath)) {
                                    const serviceMetadata = (await geServiceMetadataJson(serviceMetadataPath)); // eslint-disable-line @typescript-eslint/no-explicit-any
                                    if (serviceMetadata?.mobile?.destinations &&
                                ...
                                console.error("[MDK MCP Server] Warning: Could not read destination from service metadata:", error);
                        ...
                        if (mdkToolsPath) {
                            ...
                            const externalsString = externals.length > 0
                ...
                const errorMessage = error instanceof Error ? error.message : String(error);
                if (errorMessage.toLowerCase().includes("cloud foundry token") ||
                    ...
                    case "search-samples": {
                        ...
                        try {
                            ...
                            for (let i = 0; i < results.length; i++) {
                                ...
                                resultText += `## Result ${i + 1} (Relevance: ${similarity}%)\n\n`;
                                resultText += `\`\`\`\n${content}\n\`\`\`\n\n`;
                                resultText += `---\n\n`;
                            }
                            return {
        ...
        case "mdk-fetch-mobile-metadata": {
            try {
                ...
                const validatedArgs = validateToolArguments("mdk-fetch-mobile-metadata", request.params.arguments || {});
                const projectPath = validatedArgs.folderRootPath;
                const appId = validatedArgs.appId;
                const destination = validatedArgs.destination;
                const pathSuffix = validatedArgs.pathSuffix || "";
                const landscapeType = validatedArgs.landscapeType || "Standard";
                // Refresh CF OAuth token before fetching metadata
                const refreshSuccess = refreshCFToken();
                if (!refreshSuccess) {
                    console.error("[MDK MCP Server] Warning: Failed to refresh CF token, continuing with existing token");
                }
                ...
                if (!apiUrl) {
                    throw new Error("Failed to get Mobile Services API URL from CF configuration");
                }
                // Create Mobile Services client and fetch metadata
                const client = createMobileServicesClient(apiUrl, cfToken);
                const metadata = await client.fetchMetadata(appId, destination, pathSuffix);
                ...
                fs.writeFileSync(metadataFilePath, JSON.stringify(metadataStructure, null, 4));
                return {
                    content: [
                        {
                            type: "text",
...
export default async function run(_options = {}) {
    ...
    if (!fs.existsSync(versionEmbeddingPath)) {
        console.error(`[MDK MCP Server] Initializing schema embeddings for version ${serverConfig.schemaVersion}...`);
    ...
    const rulembeddingPath = path.join(projectRoot, `build/embeddings/rule-chunks.bin`);
