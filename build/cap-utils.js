...
import { getServicesInfo, getServiceEdmx, extractEntitySetsFromServicesInfo, isArtifactManagementAvailable, } from "./artifact-management-wrapper.js";
/**
...
export function isCapProject(projectPath) {
    ...
    if (fs.existsSync(packageJsonPath)) {
        try {
            ...
            if ((packageJson.dependencies && "@sap/cds" in packageJson.dependencies) ||
 ...
 */
export async function getCapServicesInfo(projectPath) {
 ...
 */
export async function getCapServiceEdmx(projectPath, serviceName) {
    if (!isArtifactManagementAvailable()) {
        console.warn("Artifact management not available");
        return "";
    }
 ...
 */
export async function extractEntitySetsFromCapProject(projectPath) {
    try {
 ...
 */
export async function getCapProjectInfo(projectPath) {
    const result = {
        isCapProject: false,
    ...
    const metadataPath = path.join(result.mdkProjectPath || projectPath, ".service.metadata");
 ...
 */
export async function generateServiceMetadataFromCap(capProjectPath, serviceName, applicationId) {
    try {
...
export async function getCapMdkConfig(projectPath) {
