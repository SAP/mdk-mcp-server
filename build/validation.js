...
export const ValidationSchemas = {
    ...
    documentationOperation: z.enum(["search", "component", "property", "example", "search-samples"], {
    ...
    externals: z
        ...
        .refine(arr => arr.every(pkg => /^[@a-zA-Z0-9/_-]+$/.test(pkg)), "External package names contain invalid characters")
        .default([]),
    applicationId: z
        .string()
    ...
    odataContent: z
        ...
        .refine(str => str.includes("<?xml") || str.includes("<edmx:Edmx"), "OData content must be valid XML metadata"),
    ...
    mobileAppId: z
        ...
        .regex(/^[a-zA-Z0-9._-]+$/, "Mobile Services application ID contains invalid characters"),
    pathSuffix: z
        .string()
    ...
    cfOrg: z
        ...
        .regex(/^[a-zA-Z0-9._-]+$/, "CF organization name contains invalid characters")
        .optional(),
    cfSpace: z
        .string()
...
export function validateToolArguments(toolName, args) {
    ...
    switch (toolName) {
        case "mdk-create": {
            ...
            if ((args.cfOrg !== undefined && args.cfSpace === undefined) ||
        ...
        case "mdk-manage":
            ...
            if (args.externals !== undefined) {
                validatedArgs.externals = ValidationSchemas.externals.parse(args.externals);
            }
            else {
        ...
        case "mdk-save-metadata":
            validatedArgs.folderRootPath = validateSecurePath(String(args.folderRootPath));
            validatedArgs.applicationId = ValidationSchemas.applicationId.parse(args.applicationId);
            validatedArgs.destinationName = ValidationSchemas.destinationName.parse(args.destinationName);
            validatedArgs.odataContent = ValidationSchemas.odataContent.parse(args.odataContent);
            break;
        case "mdk-list-mobile-apps":
            validatedArgs.landscapeType = ValidationSchemas.landscapeType.parse(args.landscapeType || "Standard");
            break;
        ...
        case "mdk-fetch-mobile-metadata":
            validatedArgs.folderRootPath = ValidationSchemas.folderRootPath.parse(args.folderRootPath);
            validatedArgs.appId = ValidationSchemas.mobileAppId.parse(args.appId);
            validatedArgs.destination = ValidationSchemas.destinationName.parse(args.destination);
            validatedArgs.pathSuffix = ValidationSchemas.pathSuffix.parse(args.pathSuffix || "");
            validatedArgs.landscapeType = ValidationSchemas.landscapeType.parse(args.landscapeType || "Standard");
            break;
        default:
