...
export class MobileServicesClient {
    ...
    async listApplications() {
        try {
            ...
            return apps.filter(app => Array.isArray(app.services) &&
                app.services.some(s => s.name === "proxy"));
        }
        catch (error) {
            if (axios.isAxiosError(error)) {
                throw new Error(`Failed to list Mobile Services applications: ${error.message}`);
            }
            throw error;
        }
    }
     ...
     */
    async fetchMetadata(appId, destination, pathSuffix = "") {
        try {
            // Build metadata URL
            ...
            throw new Error(`Failed to fetch metadata: Status ${body.status?.code || "unknown"}`);
        }
        catch (error) {
            if (axios.isAxiosError(error)) {
                if (error.response?.status === 404) {
                    throw new Error(`Destination '${destination}' not found in Mobile Services app '${appId}'. ` +
     ...
     */
    async destinationExists(appId, destination) {
        try {
 ...
 */
export function createMobileServicesClient(adminAPI, cfToken) {
